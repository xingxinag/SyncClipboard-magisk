...
func TestIsInvalidClipboardOutput_ProviderErrors(t *testing.T) {
	tests := []struct {
		name  string
		input string
	}{
		{
			...
			input: "Error while accessing provider:clipboard java.lang.IllegalStateException: Could not find provider: clipboard",
		},
		{
			name:  "provider access failed",
			input: "Error while accessing provider",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if !isInvalidClipboardOutput(tt.input) {
				t.Fatalf("expected invalid output, got valid: %q", tt.input)
			}
		})
	}
}
